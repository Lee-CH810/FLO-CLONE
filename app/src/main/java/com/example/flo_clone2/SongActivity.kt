...
//        editor.putString("songData", songJson) // 에디터에 song이라는 태그로 songJson 삽입
    ...
    private fun setLike(isLike: Boolean) {
        ...
        songDB.songDao().updateIsLikeById(!isLike, songs[nowPos].id) // DB에 바뀐 좋아요에 대한 정보 업데이트

        /* 바뀐 Song 데이터를 렌더링 */
        if(!isLike) {
            binding.songLikeIv.setImageResource(R.drawable.ic_my_like_on)
            Toast.makeText(this, "보관함에 추가", Toast.LENGTH_SHORT).show()
        } else {
            binding.songLikeIv.setImageResource(R.drawable.ic_my_like_off)
            Toast.makeText(this, "보관함에서 삭제", Toast.LENGTH_SHORT).show()
        }
...
//        }
        ...
        val curPos = spf.getInt("currentPosition", 0)
    ...
    private fun setPlayer(song: Song) {
        ...
        /* Song 데이터에 따라 좋아요 버튼 표시 */
        Log.d("Like", song.isLike.toString())
        if(song.isLike) {
            binding.songLikeIv.setImageResource(R.drawable.ic_my_like_on)
        } else {
            binding.songLikeIv.setImageResource(R.drawable.ic_my_like_off)
